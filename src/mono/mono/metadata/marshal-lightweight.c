...
emit_native_wrapper_ilgen (MonoImage *image, MonoMethodBuilder *mb, MonoMethodSignature *sig, MonoMethodPInvoke *piinfo, MonoMarshalSpec **mspecs, gpointer func, MonoNativeWrapperFlags flags)
{
	...
	/*
	 * The tmp_locals array is used to store the local variable for regular converted arguments
	 * and the tmp_struct_locals to store the local variables for the lowered Swift struct elements
	 */ 
	for (i = 0; i < sig->param_count; i ++) {
